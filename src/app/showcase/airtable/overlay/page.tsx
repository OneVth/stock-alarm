...
export default function OverlayShowcase() {
  return (
    <div className="space-y-12">
      <div>
        <p className="at-caption text-muted-foreground">Overlay & Popup</p>
        <h1 className="at-section mt-1">오버레이 & 팝업</h1>
        <p className="at-body mt-2 text-muted-foreground">7 components</p>
      </div>
            ...
            <SheetContent side="left">
              ...
              <nav className="mt-6 space-y-1">
                {["대시보드", "알림 이력", "설정"].map((item) => (
                  <div
                    key={item}
                    className="flex items-center gap-3 rounded-lg px-3 py-2 at-body hover:bg-accent cursor-pointer"
                  >
                    {item}
                  </div>
                ))}
        ...
        <Popover>
          ...
          <PopoverContent className="w-72">
            <div className="space-y-3">
              <div>
                <p className="at-body font-semibold">삼성전자</p>
                <p className="at-caption text-muted-foreground">005930 · KOSPI</p>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <p className="at-caption text-muted-foreground">현재가</p>
                  <p className="at-body font-medium">77,200</p>
                </div>
                <div>
                  <p className="at-caption text-muted-foreground">등락률</p>
                  <p className="at-body font-medium" style={{ color: "#16a34a" }}>
                    +1.58%
                  </p>
                </div>
                <div>
                ...
                <div>
                  <p className="at-caption text-muted-foreground">임계값</p>
                  <p className="at-body font-medium">±5%</p>
                </div>
              </div>
            </div>
          </PopoverContent>
      ...
      >
        <ContextMenu>
          <ContextMenuTrigger className="flex h-24 w-full items-center justify-center rounded-xl border border-dashed border-border at-body text-muted-foreground cursor-default select-none">
            여기를 우클릭하세요
          </ContextMenuTrigger>
          <ContextMenuContent>
            <ContextMenuItem>
        ...
        <div className="flex flex-wrap gap-4">
          {[
            ...
            <HoverCard key={item.code}>
              <HoverCardTrigger render={<Button variant="link" className="p-0 h-auto at-body" />}>
                {item.name}
              </HoverCardTrigger>
              <HoverCardContent className="w-72">
                <div className="space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="at-body font-semibold">{item.name}</p>
